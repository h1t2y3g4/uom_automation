...
调用直接使用 `--headless` 模式，默认用无头浏览器。开发此项目才用有头模式。
...
- 遇到新的空域申请任务，默认工作流是：先用高德地图定位地点并拿到经纬度，再计算待申请 polygon，然后用 `cli/uom_airspace_probe.py query` 查询是否适飞，最后再根据查询结果生成申请，经人工审核同意后再提交申请。
- 默认高度策略：如果整块 polygon 都在适飞区（`inside_suitable`），默认按 500m 申请；如果哪怕只有一部分不在适飞区（如 `partial_overlap`、`outside_suitable`），默认按 120m 申请
- 默认范围策略：如果只是给某个地点，没有描述范围，默认按照半径10km拟合的多边形申请。如果是点A到点B的飞行走廊，则默认按照走廊中心线向两侧各扩展5km，中心线的两端的延长线延长5km半径的扇形，组合后拟合再申请。
