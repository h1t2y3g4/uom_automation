...
def main():
    ...
    try:
        ...
        if latest_err:
            ...
            context.close()
            raise SystemExit(1)
        next_beg, next_end = core.get_debug_target_plan_time(detail['planBeg'], detail['planEnd'])
        ...
        print(json.dumps(core.sanitize_for_json(inspect), ensure_ascii=False, indent=2)[:12000])
        print('\n这个脚本不会自动提交，也不会等待你输入回车。')
        print('请重点观察：')
        print('1. precheck 里 fields 是否仍然包含 uavs / drivers')
        print('2. inspect 里 addButtons / dialogsAfterUav / dialogsAfterDriver / vueAfterUav / vueAfterDriver 有没有选择器线索')
